...
class MultiTaskTransformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        price = 0.7 * self.price_head(shared) + 0.3 * self.price_skip(x)
        direction = 0.7 * self.direction_head(shared) + 0.3 * self.direction_skip(x)
...
class MultiTaskPredictor:
    ...
    def _load_model(self):
        ...
        self.model = MultiTaskTransformer(
            input_dim=input_dim, d_model=64, num_heads=4,
            num_layers=3, d_ff=256, dropout=0.3, max_seq_length=self.seq_length
        ).to(self.device)
    ...
    def predict_next(self):
        ...
        with torch.no_grad():
            ...
            price_pred_scaled, direction_logits = self.model(input_seq)
    ...
    def predict_multi_horizon(self, horizons=[15, 60, 240, 720, 1440, 4320, 10080]):
        ...
        with torch.no_grad():
            for minutes in horizons:
                ...
                price_pred_scaled, dir_prob_logits = self.model(input_seq)
                
                price_pred = self.price_scaler.inverse_transform(
                    price_pred_scaled.cpu().numpy().reshape(-1, 1)
                )[0, 0]
                
                dir_prob = torch.sigmoid(dir_prob_logits).item()
