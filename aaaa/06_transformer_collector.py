        ...
        try:
            ...
            if 'result' in data and 'XXBTZUSD' in data['result']:
                ...
                price_data = {
                    ...
                    'high': float(ticker_data['h'][0]),  # Use the first value for high
                    'low': float(ticker_data['l'][0]),   # Use the first value for low
                    'close': float(ticker_data['c'][0]),
                    'volume': float(ticker_data['v'][0])  # Use the first value for volume
                }
                return price_data
            else:
                logging.error("Unexpected response structure: 'result' or 'XXBTZUSD' missing.")
        except requests.exceptions.ConnectionError as conn_err:
            logging.error(f"Connection error: {conn_err}. Retrying...")
            time.sleep(5)  # Wait before retrying
        except requests.exceptions.RequestException as req_err:
            logging.error(f"Request error fetching Kraken price: {req_err}")
        except KeyError as key_err:
            logging.error(f"Unexpected response structure: {key_err}")
        except Exception as e:
            logging.error(f"General error fetching Kraken price: {e}")
        return None
