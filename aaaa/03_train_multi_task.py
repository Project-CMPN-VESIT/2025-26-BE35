...
class MultiTaskTransformer(nn.Module):
    ...
    """
    def __init__(self, input_dim, d_model=256, num_heads=16, num_layers=6, 
                 d_ff=1024, dropout=0.1, max_seq_length=100):
        super().__init__()
        ...
        self.direction_skip = nn.Linear(d_model, 1)
...
def train_multi_task_model(
    ...
    model = MultiTaskTransformer(
        ...
        d_model=64,           # Reduced from 128 to prevent noise-memorization
        num_heads=4,          # Reduced from 8
        num_layers=3,         # Reduced from 4
        d_ff=256,             # Reduced from 512
        dropout=0.3,          # Increased for heavier financial regularization
    ...
    # Cosine annealing extended to search deeper
    ...
    for epoch in range(epochs):
        ...
        for sequences, price_targets, dir_targets in train_loader:
            ...
            with autocast():
                price_pred, dir_pred_logits = model(sequences)

                loss_price = price_criterion(price_pred, price_targets)
                loss_direction = direction_criterion(dir_pred_logits, dir_targets)
        ...
        with torch.no_grad():
            for sequences, price_targets, dir_targets in val_loader:
                ...
                dir_pred_prob = torch.sigmoid(dir_pred_logits)
                val_dir_preds.extend((dir_pred_prob > 0.5).cpu().numpy())
    ...
    with torch.no_grad():
        for sequences, price_targets, dir_targets in test_loader:
            ...
            dir_pred_prob = torch.sigmoid(dir_pred_logits)
            test_dir_preds.extend((dir_pred_prob > 0.5).cpu().numpy())
            
            test_price_targets_scaled.extend(price_targets.cpu().numpy())
            test_dir_targets.extend(dir_targets.cpu().numpy())
